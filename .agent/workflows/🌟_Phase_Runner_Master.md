...
## EXECUTION SEQUENCE
1. **Iteration Loop**: `03-execute-iteration` を起動する。
   - **Continuous Sync**: ステップの開始時、および `04-verify-and-debug` での不合格/エラー発生時は、**完了を待たずに即時** `master-reporter` を起動し、ポータルとダッシュボードに現状を公開する。
   - テストをパスするまで自律的にループ、または滞留状況を継続的に報告する。
2. **Dashboard Update**: フェーズ完了後、および各報告時に `master-reporter` を起動し、`00_Master_Report.md` の進捗状況とGanttチャートを最新化する。
